...
for stepV in range(startSkipV,endSkipV+1): #counts the V steps between the start and end cut parameters and starts iterating for each one
    ...
    for stepU in range(startSkipU,endSkipU+1):  #counts the U steps between the start and end cut parameters and starts iterating for each one
        ...
        if stepV < baseLoops:
            x= math.sin(u)*v #Here we set a function for the X value
            y= math.cos(u)*v#Here we set a function for the Y value 
            z=0
        else:
            x= math.sin(u)*r #Here we set a function for the X value
            y= math.cos(u)*r#Here we set a function for the Y value 
            z = (v  + ((u*stepSizeV)/6.2831)-(stepSizeV/2)) -stepSizeV*baseLoops
